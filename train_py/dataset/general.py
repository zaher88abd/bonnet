...
def dir_to_data(directory, label_map, label_remap, new_shape=None, force_remap=False):
    ...
    r_img_dir = directory + '/rimg/'
    r_img_remap_dir = r_img_dir + '/remap/'
    r_label_dir = directory + '/rlbl/'
    r_label_remap_dir = r_label_dir + '/remap/'
    ...
    rimages = [f for f in listdir(r_img_dir)
               if isfile(join(r_img_dir, f))]
    rlabels = [f for f in listdir(r_label_dir)
               if isfile(join(r_label_dir, f))]
    ...
    for f in images[:]:
        if os.path.splitext(f)[0] not in [os.path.splitext(l)[0] for l in labels[:]]:
            ...
            l = cv2.imread(label_dir + os.path.splitext(f)
                           [0] + ".png", 0)  # open label as grayscale
            h, w = l.shape
    ...
    # check if image has a corresponding label, otherwise warn and continue
    for f in rimages[:]:
        if os.path.splitext(f)[0] not in [os.path.splitext(l)[0] for l in rlabels[:]]:
            # warn
            print("Image file %s has no label, GIMME DAT LABEL YO! Ignoring..." % f)
            # ignore image
            rimages.remove(f)
            ...
            # calculate class content in the image
            # print("Calculating label content of label %s"%f)
            l = cv2.imread(r_label_dir + os.path.splitext(f)
                           [0] + ".png", 0)  # open label as grayscale
            h, w = l.shape
            total_pix += h * w  # add to the total count of pixels in images
            # print("Number of pixels in image %s"%(h*w))
    ...
    # loop labels checking rogue labels with no images (magic label from ether)
    for f in rlabels[:]:
        if os.path.splitext(f)[0] not in [os.path.splitext(i)[0] for i in rimages[:]]:
            # warn
            print("Label file %s has no image, IS THIS MAGIC?! Ignoring..." % f)
            # ignore image
            rlabels.remove(f)
    ...
    # remove remap folders to create them again
    if force_remap and os.path.exists(r_label_remap_dir):
        shutil.rmtree(r_label_remap_dir)
    if force_remap and os.path.exists(r_img_remap_dir):
        shutil.rmtree(r_img_remap_dir)
    ...
    if not os.path.exists(label_remap_dir):
        ...
        for f in labels:
            ...
            if(new_shape is not None):
                lbl = cv2.resize(
                    lbl, new_shape, interpolation=cv2.INTER_NEAREST)
    ...
    # remap all labels to [0,num_classes], otherwise it breaks the crossentropy
    if not os.path.exists(r_label_remap_dir):
        print("Cross Entropy Right remap non existent, creating...", label_remap)
        os.makedirs(r_label_remap_dir)
        for f in rlabels:
            lbl = cv2.imread(r_label_dir + f, 0)
            if(new_shape is not None):
                lbl = cv2.resize(
                    lbl, new_shape, interpolation=cv2.INTER_NEAREST)
            for key in label_remap:
    ...
    new_images = []
    if not os.path.exists(r_img_remap_dir):
        print("Jpeg remap non existent, creating...")
        os.makedirs(r_img_remap_dir)
        for f in rimages:
            img = cv2.imread(r_img_dir + f, cv2.IMREAD_UNCHANGED)
            f = os.path.splitext(f)[0] + '.jpg'
            new_images.append(f)
            if(new_shape is not None):
                img = cv2.resize(
                    img, new_shape, interpolation=cv2.INTER_LINEAR)
            cv2.imwrite(r_img_remap_dir + f, img)
    else:
        for f in images:
            f = os.path.splitext(f)[0] + '.jpg'
            new_images.append(f)
