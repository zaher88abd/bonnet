...
class AbstractNetwork:
    ...
    def loss_f(self, lbls_pl, logits_train, gamma_focal=2, w_t="log", w_d=1e-4):
        ...
        with tf.variable_scope("loss"):
            ...
            gamma = np.full(onehot_mat.get_shape().as_list(),
                            fill_value=gamma_focal)
        ...
        if self.ckpt and self.ckpt.model_checkpoint_path:
            ...
            try:
                ...
                restore_saver.restore(
                    self.sess, self.ckpt.model_checkpoint_path)
                ...
                print('*' * 80)
                print(
                    "Keeping classifier random, to see if this helps (also keeping all the training stuff the same)")
                restore = []
                not_restore = []
                restore.extend(tf.global_variables(scope='model'))
    ...
    def predict_kickstart(self, path, batchsize=1, data_format="NCHW"):
        ...
        gpu_options = tf.GPUOptions(
            allow_growth=True, force_gpu_compatible=True)
    ...
    def freeze_graph(self, path=None, verbose=False):
        ...
        out_opt_tensorRT_graph_name = os.path.join(
            self.log, 'optimized_tRT.pb')
        uff_opt_tensorRT_graph_name = os.path.join(
            self.log, 'optimized_tRT.uff')
    ...
    def pix_acc_from_histogram(self, hist):
        ...
        # calculate IoU
        per_class_iou = np.diag(hist) / (hist.sum(1) +
        ...
        for i in range(self.num_classes):
            ...
            for j in range(1, num_components):  # 0 is background (useless)
                ...
                if stats[j][cv2.CC_STAT_AREA] > 50:
                    ...
                    # mask the prediction
                    masked_prediction = np.ma.masked_array(
                        mask, mask=comp_mask)
    ...
    def individual_accuracy(self, mask, label):
        ...
        print('   Mean Accuracy: %0.04f, Mean IoU: %0.04f' %
              (mean_acc, mean_iou))
    ...
    def dataset_accuracy(self, dataset, batch_size, ignore_last=False):
        ...
        for step in range(steps_per_epoch):
            ...
            for g in range(0, self.n_gpus):
                ...
                pred = self.sess.run(
                    self.logits_valid_list[g], feed_dict=feed_dict)
                ...
                for idx in range(0, batch_size):
                    ...
                    if proper_w != w or proper_h != h:
                        label = ad.resize(
                            label, [proper_h, proper_w], neighbor=True)
    ...
    def training_dataset_accuracy(self, dataset, batch_size, batch_size_gpu,
        ...
        num_examples = steps_per_epoch * batch_size
        pix_hist = np.zeros(
            (self.num_classes, self.num_classes), dtype=np.float32)
        for step in range(steps_per_epoch):
            feed_dict, names = self.fill_feed_dict(
            ...
            for g in range(0, self.n_gpus):
                for idx in range(0, batch_size_gpu):
                    ...
                    if self.TRAIN["save_imgs"]:
                        ...
                        color_mask = util.prediction_to_color(
                            mask, self.DATA["label_remap"], self.DATA["color_map"])
                        print("ColorMask", color_mask.shape)
                        color_label = util.prediction_to_color(
                            label, self.DATA["label_remap"], self.DATA["color_map"])
                        print("ColorLabel", color_label.shape)
                        path_to_save = self.log + '/predictions/'
                        if not tf.gfile.Exists(path_to_save):
                            tf.gfile.MakeDirs(path_to_save)
                        cv2.imwrite(path_to_save + dataset.name + '_' + str(name),
    ...
    def train(self, path=None):
        ...
        with tf.Graph().as_default(), tf.device('/cpu:0'):
                ...
                with tf.variable_scope("model"):
                    for i in range(self.n_gpus):
                        with tf.device(self.assign_to_device('/gpu:%d' % i)):
                            ...
                            # graph
                            logits_train, _, _ = self.build_graph(
                                img_pl, True)  # train
                            ...
                            with tf.name_scope("loss_%d" % i):
                                ...
                                if self.TRAIN["grads"] == "speed":
                                    ...
                                    print(
                                        "Gradient option not supported. Check config")
                                grads_and_vars = list(
                ...
                with tf.variable_scope("model", reuse=True):
                    for i in range(self.n_gpus):
                        with tf.device('/gpu:%d' % i):
                            ...
                            logits_valid, _, _ = self.build_graph(
                                img_pl, False)  # test
            ...
            with tf.variable_scope("accuracies"):
                self.train_accuracy = tf.Variable(
                    0.0, name="train", trainable=False)
                self.validation_accuracy = tf.Variable(
                    0.0, name="validation", trainable=False)
                self.test_accuracy = tf.Variable(
                    0.0, name="test", trainable=False)
            ...
            with tf.variable_scope("loss_value"):
                ...
                self.train_loss = tf.Variable(
                    0.0, name="train_loss", trainable=False)
            ...
            acc_report_steps = int(
                self.TRAIN["acc_report_epochs"] * steps_per_epoch)
            ...
            for self.step in range(max_steps):
                ...
                # print("test point zaher1")
                # fill in the dictionaries
                start_time = time.time()
                print("test point zaher01")
                print("test point",np.array(self.dataset.train).shape)
                print("test point", np.array(self.img_pl_list).shape,',' , np.array(self.lbls_pl_list).shape)

                feed_dict, _ = self.fill_feed_dict(self.dataset.train,
                                                   self.img_pl_list,
                                                   self.lbls_pl_list,
                                                   self.batch_size_gpu)
                print("test point zaher0")
                duration_get_batch = time.time() - start_time
                # Run one step of the model in all gpus
                start_time = time.time()
                print("test point zaher1")
                print("train_op", type(self.train_op))
                # print("train_op", self.train_op.shape)
                # print("self.loss", self.loss.shape)
                _, self.loss_value = self.sess.run(
                    [self.train_op, self.loss], feed_dict=feed_dict)
                print("test point zaher2")
                ...
                if self.step % self.TRAIN["summary_freq"] == 0:
                    ...
                    if self.TRAIN["ignore_crap"]:
                        pix_h = pix_h[:-1, :-1]
                    mean_acc, mean_iou, _, _, _ = self.pix_acc_from_histogram(
                        pix_h)
                    ...
                    self.summary_str = self.sess.run(
                ...
                if self.step % acc_report_steps == 0 or (self.step + 1) == max_steps:
                    ...
                    # assign the value to the nodes
                    self.sess.run([acc_op, iou_op])
                    ...
                    if m_acc > self.best_acc_validation:
                        ...
                        # report to user
                        print(
                            "Best validation accuracy yet, saving network checkpoint")
                    ...
                    if m_iou > self.best_iou_validation:
                        ...
                        # report to user
                        print(
                            "Best validation mean IoU yet, saving network checkpoint")
    ...
    def fill_feed_dict(self, data_set, img_pl_list, lbls_pl_list, batch_size):
        ...
        print("test point fill_feed_dict", np.array(feed_dict).shape)
        print("test point img_pl_list", np.array(img_pl_list).shape)
        print("test point images_feed", np.array(images_feed).shape)
        print("test point labels_feed", np.array(labels_feed).shape)
